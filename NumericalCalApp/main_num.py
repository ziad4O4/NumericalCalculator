...
def false_window():
    ...
    calculate_button = ctk.CTkButton(window, text="Calculate",fg_color="#F999B7", hover_color="#F9C5D5", command=calculate)
    calculate_button.place(x=50, y=220)
...
def gje_window():
    ...
    matrix_label_1.place(x=40, y=220, width=80, height=20)
    matrix_label_2 = ctk.CTkLabel(window, text="")
    matrix_label_2.place(x=100, y=220, width=80, height=20)
    matrix_label_3 = ctk.CTkLabel(window, text="")
    matrix_label_3.place(x=180, y=220, width=80, height=20) 
...
def cramer_window():
    ...
    for i in range(3):
        ...
        for j in range(4):
            xlbl = ctk.CTkLabel(window, text=f"X{j + 1}")
            xlbl.place(x=100 * j + 70, y=30 * i)
            entry = ctk.CTkEntry(window, width=40, height=1, border_color='#3C68A7')  
            entry.place(x=100 * j + 10, y=30 * i)
            row.append(entry)
        entry_matrix.append(row)


    calculate_button = ctk.CTkButton(window, text="Calculate", width=220, fg_color="#F999B7", hover_color="#F9C5D5", corner_radius=10,command=calculate)
...
def gradiant_window():
    def Gradiant(func, xo, yo):
        x ,y =sp.symbols('x y')   
        f=sp.sympify(func)
        fdashx=sp.diff(f,x)
        fdashx_lbl.configure(text=f"f'(x)= {fdashx}")
        fdashy=sp.diff(f,y)
        fdashy_lbl.configure(text=f"f'(y)= {fdashy}")
        p=fdashx.subs({x:xo , y:yo})
        k=fdashy.subs({x:xo , y:yo})
        
        gradiant=str(p) +"i" + str(k)+"j"
        gradient_lbl.configure(text=f"gradiant = {gradiant}")
        
        magnitude=((p**2)+(k**2))**0.5
        magnitude_lbl.configure(text=f"magnitude = {magnitude}")
        ...
        if cyta <0:
            ...
            cyta = cyta
        cyta_lbl.configure(text=f"cyta = {cyta}")

    def calculate():
        fn = function_entry.get()
        xo = float(xo_entry.get())
        yo = float(yo_entry.get())
        root= Gradiant(fn, xo, yo)

    window=ctk.CTk()  
    window.geometry('720x730')
    window.title("Gradiant Method")
    window.resizable(False,False)

    # Function entry
    function_entry = ctk.CTkEntry(window,placeholder_text='Enter the function F(Xi)',border_color='#FEE3EC',width=440)
    function_entry.place(x=40, y=100)

        # Xo entry
    xo_entry = ctk.CTkEntry(window,placeholder_text='Enter the value of Xo',border_color='#FEE3EC')
    xo_entry.place(x=40, y=150)

        # yo entry
    yo_entry = ctk.CTkEntry(window,placeholder_text='Enter the value of yo',border_color='#FEE3EC')
    yo_entry.place(x=245, y=150)

        # buttons
    calculate_button = ctk.CTkButton(window, text="Calculate",width=220,fg_color= '#F999B7', hover_color="#F9C5D5" ,corner_radius=12 ,command=calculate)
    calculate_button.place(x=50, y=220)

    fdashx_lbl=ctk.CTkLabel(window, text="",font=('Arial ',16))
    fdashx_lbl.place(x=20, y=140)

    fdashy_lbl=ctk.CTkLabel(window, text="",font=('Arial',16))
    fdashy_lbl.place(x=20, y=190)

    gradient_lbl=ctk.CTkLabel(window, text="",font=('Arial',16))
    gradient_lbl.place(x=20, y=250)

    magnitude_lbl=ctk.CTkLabel(window, text="",font=('Arial ',16))
    magnitude_lbl.place(x=20, y=300)

    cyta_lbl=ctk.CTkLabel(window, text="",font=('Arial ',19))
    cyta_lbl.place(x=20, y=390)
...
gradiant_button = ctk.CTkButton(root, text="Gradiant", height=30, width=240,  fg_color="#F999B7", hover_color="#F9C5D5", corner_radius=8, anchor=ctk.CENTER,
                                    command=gradiant_window)
gradiant_button.pack(pady=10)
